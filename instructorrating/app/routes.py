...
from app.db_access import import_reviews, import_professors
import pandas as pd
from app.models import Professor, Department, Course
...
def leaderboard():
    ...
    qry = Professor.query.filter(Professor.nrev>=15).order_by(Professor.score.desc()).limit(20)
    items = qry.all()
...
def populatereviews():
    df = pd.read_csv('../data/df_reviews.csv', index_col=0)
    import_reviews(df)
    return redirect('/index')
